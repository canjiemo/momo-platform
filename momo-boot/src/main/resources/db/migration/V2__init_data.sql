...
SELECT NULL, 100, m.id FROM (VALUES
...
WHERE NOT EXISTS (
    SELECT 1 FROM public.sys_role_menu WHERE tenant_id IS NULL AND role_id = 100 AND menu_id = m.id
);
...
WHERE NOT EXISTS (SELECT 1 FROM public.sys_file_config WHERE storage_type = 'local' AND delete_flag = 0);

INSERT INTO public.sys_file_config (config_name, storage_type, is_active, config, remark, create_by, create_time, update_by, update_time, delete_flag)
SELECT 'MinIO 存储', 'minio', 0,
       '{"endpoint":"http://minio:9000","bucket":"momo-platform","accessKey":"","secretKey":"","publicBucket":true,"presignedExpireSeconds":3600}',
       '生产环境使用 MinIO 对象存储', 'system', NOW(), 'system', NOW(), 0
WHERE NOT EXISTS (SELECT 1 FROM public.sys_file_config WHERE storage_type = 'minio' AND delete_flag = 0);
